...
def select_stream(params):
    ...
    if not data or season not in data['seasons'] or episode not in data['seasons'][season]:
        xbmcgui.Dialog().notification('YaWSP', 'Streamy nenalezeny', xbmcgui.NOTIFICATION_WARNING)
        return
    episode_data = data['seasons'][season][episode]
    streams = episode_data.get('streams', [episode_data])
    items = []
    for s in streams:
        size_mb = int(s.get('size', '0')) // (1024*1024)
        name = s['name']
        fmt = ''
        lang = ''
        subs = ''
        # Format
        m = re.search(r'(\b(1080p|720p|480p|2160p|x264|x265|hevc|web-dl|hdtv|dvdrip|bluray|webrip|hdr)\b)', name, re.IGNORECASE)
        if m:
            fmt = m.group(1)
        # Jazyk
        m = re.search(r'(\bcz\b|\bsk\b|\ben\b|dabing|titulky)', name, re.IGNORECASE)
        if m:
            lang = m.group(1)
        # Titulky a jazyk titulku
        if re.search(r'(srt|sub|tit|titulky)', name, re.IGNORECASE):
            subs = 'titulky'
            # Zkusim najit jazyk titulku
            m = re.search(r'(cz|sk|en)[ ._-]*titulky', name, re.IGNORECASE)
            if m:
                subs += f" {m.group(1).lower()}"
        label = f"{name} ({size_mb} MB)"
        if fmt:
            label += f" | {fmt}"
        if lang:
            label += f" | {lang}"
        if subs:
            label += f" | {subs}"
        items.append(label)
    idx = xbmcgui.Dialog().select('Vyber stream', items)
    if idx >= 0:
        s = streams[idx]
        xbmc.executebuiltin(f'RunPlugin({get_url(action="play", ident=s["ident"], name=s["name"] )})')
...
def router(paramstring):
    ...
    if params:
        ...
        elif params['action'] == 'select_stream':
